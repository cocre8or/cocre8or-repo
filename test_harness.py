...
def check_testresult(client, test, expected_status=200):
    ...
    time.sleep(3)
    try:
    	# Open the log file to read the status, and then wipe out afterwards.
...
def check_file(file_path, should_exist=True):
    # Add some coloring and results on fail.
    start = "\033[92m" if os.path.exists(
        file_path) == should_exist else "\033[91m"
    end = "exists" if should_exist else "not exists"
    end += "\033[0m"
...
def main():
	...
	try:
		...
		check_testresult(1, USER_1 + " checkin Integrity")
